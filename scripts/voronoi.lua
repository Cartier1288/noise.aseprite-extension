...
local function voronoi_graph(points, width, height, dfunc, offsetx, offsety)
  ...
  for x = offsetx, width - 1 do
    for y = offsety, height - 1 do
...
local function voronoi(seed, width, height, length, options, loop)
  ...
  local expand = options.expand
  local npoints = math.random(options.points[1], options.points[2])

  local points = {}

  -- site generation area
  local pwidth = width * expand
  local pheight = height * expand
  ...
  if options.relax then
    for i = 1, options.relax_steps do
      local graph = voronoi_graph(points, pwidth, pheight, options.distance_func, pleft, ptop)
  ...
  -- todo: maybe sample from the gradient instead, so that colors can be biased
  local pcolors = {}
  for p = 1, #points do
  ...
  for layer = 1, length do
    ...
    local t = 0
    if length > 1 then
      -- todo: fix one frame of pause when reaching a location...

...
local SpritePainter = require("sprite-painter")

local SpritePainterPlain = { }
setmetatable(SpritePainterPlain, SpritePainter)
...
--      lock_alpha
-- }
function SpritePainterPlain:new(opts) -- noexcept(false)
    local this = SpritePainter:new()
...
function SpritePainterPlain:_get_color_index(idx)
    return self.palette:getColor(idx)
end

function SpritePainterPlain:_get_color_uint(val)
    return Color(val)
end

function SpritePainterPlain:_get_alpha_index(idx)
...
function SpritePainterPlain:_get_alpha_uint(val)
    return app.pixelColor.rgbaA(val)
end

function SpritePainterPlain:get_color_at(x,y)
...
function SpritePainterPlain:pick_color(t)
    return utils.color_grad(t, table.unpack(self.color_range))
end

function SpritePainterPlain:load_sprite_data()
...
function SpritePainterPlain:load_sprite_alpha()
    self.alphas = { }
...
-- 
function SpritePainterPlain:bind_canvas_functions()
...
function SpritePainterPlain:init_color_grad()
...
function SpritePainterPlain:get_current_frame()
    self.layer = app.layer
    self.cel = app.cel
    if not self.layer or not self.cel then
        error(app.alert("SpritePainterPlain tried to get active layer+cel, but couldn't find one"))
    end
    self.image = self.cel.image
end

function SpritePainterPlain:load_frame(idx)
...
function SpritePainterPlain:put_pixel(x, y, color)
...
-- pixel iterator
function SpritePainterPlain:pixels()
    local i = -1
...
function SpritePainterPlain:animate(frames)
...
function SpritePainterPlain:apply_masks()
...
return {
    SpritePainterPlain = SpritePainterPlain
}
...
local voronoi_defs = {
  ...
  relax = true,
  relax_steps = 5,
  expand = 1.25, -- the factor of the viewing space by which to increase/decrease the site spawn area by, 
                 -- this avoids a noticeable compression towards the center of the viewing area if >1, 
                 -- especially with relax steps.
  threed = false,
  loop = false,
  frames = 1,
  movement = 10,            -- how much a point may move during animation
  movement_func = "LERP",   -- { "LERP", "CERP", "Smootherstep"}
  locations = 1,            -- how many times the point locations changed
}
...
local function voronoi_dlog(parent, defs)
  ...
  dlog:number { id = "min_points", label = "Min Points [1,\\infin]", text = tostring(defs.min_points) }
      ...
      :number { id = "expand", label = "Site Expand [0,\\infin]", decimals=3, text = tostring(defs.expand) }

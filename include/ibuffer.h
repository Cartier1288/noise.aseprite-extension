...
public:
    ...
    struct iterator {
        ...
        iterator(ibuffer& buffer)
            : buffer(&buffer), ptr(&buffer.elements.front()), 
              x(0), y(0), idx(0)
        { }


        iterator(ibuffer& buffer, pos_t x, pos_t y)
            : buffer(&buffer), ptr(&buffer.elements[idx]), 
              x(x), y(y), idx(buffer.pos_to_idx(x, y)) 
        { }
        ...
        // makes a copy of the current iterator/location, then increments and returns the original
        // position
    ...
    private:
        ...
        iterator(ibuffer& buffer, pointer ptr)
...
public:
    ibuffer(size_t width, size_t height, pixel def=pixel())
        : width(width), height(height), dim((float)width, (float)height), def(def),
          length(width * height), elements(length) { }

    template<typename Iterator>
    ibuffer(
        size_t width, size_t height, pixel def,
        Iterator begin, Iterator end, bool flip_x = false, bool flip_y = false
    )
        : ibuffer(width, height, def) {
        set_from(begin, end, flip_x, flip_y);
    }

    void set(pos_t x, pos_t y, pixel val) {
        elements[pos_to_idx(x,y)] = val;
    }

    void set(pos_t idx, pixel val) {
        elements[idx] = val;
    }
    ...
    inline
    size_t pos_to_idx(pos_t x, pos_t y) const {
        return y * width + x;
    }

    float2 from_norm_coords(float2 coords) const {
        return coords * dim;
    }
    ...
    inline
    size_t get_extended_length() const { return length * depth; }
    ...
    void set_from(std::vector<T> const& arr) {
        if(arr.size() != get_extended_length()) {
            throw std::invalid_argument{
                "trying to set ibuffer with an array of size differing from the buffers extended "
                "length"
            };
        }
    ...
    template<typename Iterator>
    void set_from(Iterator begin, Iterator end, bool flip_x = false, bool flip_y = false) {
        Iterator it = begin;

        for(size_t y = 0; y < height; y++) {
            size_t scanned = (flip_y ? height-y-1 : y) * width;
            for(size_t x = 0; x < width; x++) {
                size_t idx = scanned + (flip_x ? width-x-1 : x); // index into the pixel vector
                size_t eidx = idx * depth; // extended index
                ...
                unroll<depth>([&, this](auto i) {
                    if(it == end) {
                        throw std::invalid_argument {
                            "trying to initialize ibuffer from an iterable with incomplete length"
                        };
                    }
                    // copy arr[eidx..eidx+depth+1] into pixel
                    p[i] = *it;
    ...
    // similar to apply, but generates a new buffer possibly of a different type and depth
    template<typename TT=T, size_t M=depth>
    ibuffer<TT, M> map(
        std::function<typename ibuffer<TT, M>::pixel(pixel const&)> const& filter
    ) {

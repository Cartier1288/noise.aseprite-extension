...
// some of these helper functions are taken from src/app/script/luacpp.h in the
// aseprite source code

template <typename T> const char* get_classname();

#define DECLARE_LUA_CLASS(T)                                                   \
  template <> const char* get_classname<T>() { return #T; }

// if a name other than the type name should be used (used for generics, like
// larray<double>)
...
// gets the user data associated with a class declared using DECLARE_LUA_CLASS
// from the stack at index idx
template <typename T> T* get_obj(lua_State* L, int idx) {
  T* ptr = (T*)luaL_checkudata(L, idx, get_classname<T>());
  assert(typeid(*ptr) == typeid(T));
  return ptr;
}

// constructs a new class of type T at the address of the user data allocated by
// lua and returns it
...
// same as push_new but adds an extra offset to the end of the user data in case
// of dynamic size
template <typename T, typename... Args>
...
// register the name of the class to the C function that creates a new instance
// of it
...
template <typename T>
void try_get_num_field(lua_State* L, T& store, int idx, int stack,
                       const char* key) {
